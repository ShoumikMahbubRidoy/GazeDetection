#!/usr/bin/env python3
"""
High-Accuracy Gaze Tracker
Advanced techniques for iPhone-level accuracy:
- 13-point calibration (more data)
- Polynomial regression (non-linear mapping)
- Better eye center calculation
- Temporal smoothing with outlier rejection
- Per-eye gaze estimation
"""
import cv2
import numpy as np
import time
...
class HighAccuracyGazeTracker:
    def __init__(self):
        ...
        # Calibration - 13 points for better coverage
        self.calibration_mode = False
        self.calibration_data = []
        self.model_x = None  # Polynomial regression for X
        self.model_y = None  # Polynomial regression for Y
        self.poly_features = PolynomialFeatures(degree=2)
        self.current_calib_idx = 0
        
        # 13-point calibration grid (more points = better accuracy)
        margin = 0.08
    ...
    def estimate_head_pose(self, landmarks, img_shape):
        ...
        # Use more points for better head pose
        model_points = np.array([
            (0.0, 0.0, 0.0),           # Nose tip
            (-30.0, -30.0, -50.0),     # Left eye
            (30.0, -30.0, -50.0),      # Right eye
            (-20.0, 30.0, -30.0),      # Left mouth
            (20.0, 30.0, -30.0),       # Right mouth
            (0.0, -50.0, -50.0),       # Chin
            (-50.0, 0.0, -40.0),       # Left cheek
            (50.0, 0.0, -40.0)         # Right cheek
        ], dtype=np.float64)
    ...
    def estimate_gaze_advanced(self, landmarks):
        """Advanced gaze estimation using eye geometry"""
        # Left eye
        ...
        # Normalized gaze vectors (relative to eye size)
        ...
        # Also return individual eye gazes for feature vector
    ...
    def extract_features(self, head_pose, gaze_data):
        ...
        # Feature vector: [yaw, pitch, roll, avg_gaze_x, avg_gaze_y, 
    ...
    def calculate_screen_point(self, features):
        """Predict screen coordinates using trained model"""
    ...
    def smooth_point_advanced(self, point):
        ...
        threshold = np.percentile(distances, 80)  # Keep 80% closest points
        ...
        # Weighted average (recent points weighted more)
        weights = np.linspace(0.5, 1.0, len(filtered_points))
        weights = weights / weights.sum()
        
        smoothed = np.average(filtered_points, axis=0, weights=weights)
    ...
    def compute_calibration(self):
        ...
        try:
            ...
            # Train separate models for X and Y with regularization
            ...
            print(f"✓ Advanced calibration computed from {len(self.calibration_data)} points!")
            print(f"  Polynomial degree: 2, Ridge regularization")
            return True
    ...
    def run_calibration(self):
        ...
        print("\n" + "="*60)
        print("HIGH-ACCURACY CALIBRATION")
        print("="*60)
        print(f"13-point calibration for maximum accuracy")
        print("1. Look at RED circle")
        print("2. Press SPACE when ready")
        print("3. Keep looking while GREEN (collecting 40 samples)")
        print("ESC to cancel")
        print("="*60 + "\n")
        ...
        while self.calibration_mode and self.current_calib_idx < len(self.calib_screen_points):
            ...
            if waiting_for_space:
                ...
                color = (0, 255, 0)
                status = f"Collecting {len(samples)}/40"
            
            # Draw target (larger for visibility)
    ...
    def run(self):
        print("=" * 60)
        print("HIGH-ACCURACY Gaze Tracker")
        print("=" * 60)
        print(f"Screen: {self.screen_width}x{self.screen_height}")
        print("Features:")
        print("  - 13-point calibration")
        print("  - Polynomial regression")
        print("  - Advanced smoothing")
        print("  - Per-eye gaze tracking")
        print("\nControls:")
        print("  c - Start calibration (REQUIRED for accuracy)")
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                if head_pose is not None:
                    ...
                    if self.model_x is not None:
                        cv2.putText(frame, "CALIBRATED (High Accuracy)", (10, h-10),
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                    else:
                        cv2.putText(frame, "Press 'c' to calibrate for accuracy", (10, h-10),
            ...
            cv2.imshow("High-Accuracy Gaze Tracker", frame)
