#!/usr/bin/env python3
"""
OAK-D High-Accuracy Gaze Tracker
Uses RGBHandDetector for proper color handling
Near iPhone-level accuracy with OAK camera
"""
import cv2
import numpy as np
import time
...
try:
    ...
    print("ERROR: rgb_hand_detector.py not found!")
    print("Make sure rgb_hand_detector.py, HandTracker.py, and dependencies are in this folder")
    DETECTOR_AVAILABLE = False


class OAKGazeTrackerPro:
    def __init__(self):
    ...
    def compute_calibration(self):
        ...
        try:
            ...
            print(f"✓ OAK calibration complete! {len(self.calibration_data)} points")
            return True
    ...
    def run_calibration(self):
        ...
        print("\n" + "="*60)
        print("OAK CAMERA HIGH-ACCURACY CALIBRATION")
        print("="*60)
        print("13 points for near-iPhone accuracy")
        ...
        while self.calibration_mode and self.current_calib_idx < len(self.calib_screen_points):
            frame, hands, depth = self.detector.get_frame_and_hands()
            if frame is None:
                continue
    ...
    def run(self):
        ...
        print("=" * 60)
        print("OAK-D PRO GAZE TRACKER")
        print("=" * 60)
        print("Near-iPhone accuracy with OAK camera!")
        print("\nControls:")
        print("  c - Calibrate (REQUIRED)")
        print("  r - Reset")
        print("  q - Quit")
        print("=" * 60)
        ...
        self.detector = RGBHandDetector(
            fps=30,
            resolution=(1280, 720),
            pd_score_thresh=0.5,
            use_gesture=False,
            use_lm=False  # Don't need hand landmarks
        )
        
        if not self.detector.connect():
            print("✗ Failed to connect to OAK camera!")
            return
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                if head_pose is not None:
                    ...
                    # Info
                    cv2.putText(frame, f"OAK-D Gaze: ({screen_pt[0]}, {screen_pt[1]})", (10, 30),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                    
                    if self.model_x is not None:
                        cv2.putText(frame, "CALIBRATED (OAK-D Pro)", (10, h-10),
            ...
            cv2.imshow("OAK-D Gaze Tracker Pro", frame)
